...
Bool apply_operations (void *data, void *aux_data)
{
	...
	if (wd == NULL) {  /* JWT:BE SAFE B/C abUSER ENTERS THIS! */
		show_warning("e:WinCommand specified non-existing window (0x%lX)!",
				((client_item *) data)->cl);
		return False;
	}
...
void ascom_init (void)
{
	...
	/* JWT:MAKE SOME MORE WINDOW-OPS AVAILABLE TO WinCommands: */
	add_hash_item (ASCommandState.handlers, AS_HASHABLE (strdup ("shade")),
								 shade_handler);
	add_hash_item (ASCommandState.handlers, AS_HASHABLE (strdup ("stick")),
								 stick_handler);
	add_hash_item (ASCommandState.handlers, AS_HASHABLE (strdup ("fullscreen")),
								 fullscreen_handler);
	add_hash_item (ASCommandState.handlers, AS_HASHABLE (strdup ("ontop")),
								 ontop_handler);
	add_hash_item (ASCommandState.handlers, AS_HASHABLE (strdup ("focus")),
								 focus_handler);
...
select_windows_by_pattern (const char *pattern, Bool just_one,
													 Bool unselect)
{
	if (pattern == NULL)  return False;  /* JWT:BE SAFE B/C abUSER ENTERS THIS! */
	...
	if (strcmp (pattern, "^") == 0) {
		/* PATTERN == "^" - SPECIAL VALUE MEANING CURRENTLY-FOCUSED WINDOW (IF ANY): */
		int revert_to_return;
		XGetInputFocus (dpy, &focused_window, &revert_to_return);
	} else if (strlen(pattern) > 2
			&& (pattern[0] == '0' || pattern[0] == '1')
			&& (pattern[1] == 'x' || pattern[1] == 'X')) {
		/* PATTERN IS A HEX WINDOW-ID FOR A SPECIFIC WINDOW: */
		pattern += 2; /* JWT:HOPEFULLY NO ONE SEEKS FOR A WINDOW TITLED "0x..."! ;) */
		focused_window = (Window) strtol (pattern, NULL, 16);
	}
	if (focused_window != None && focused_window != 1) {  /* WE HAVE A SPECIFIC WINDOW: */
		client_item *new_item;
		new_item = safemalloc (sizeof (client_item));
		new_item->cl = focused_window;
		append_bidirelem (new_selection, new_item);
	} else {  /* WE HAVE A "PATTERN", TRY TO MATCH A WINDOW BY TITLE: */
