...
void gethelp_func_handler (FunctionData * data, ASEvent * event,
													 int module)
{
	if (event->client != NULL && ASWIN_RES_NAME (event->client) != NULL) {
		/* JWT:CHGD. TO NEXT 3 TO ALLOW HELPCOMMAND TO BE OVERRIDDEN BY ENVIRONMENT VARIABLE: char *realfilename = PutHome (HELPCOMMAND); */
		char *ashelpcommand = getenv ("ASHELPCOMMAND");
		char *realfilename = (ashelpcommand != NULL) ? PutHome (ashelpcommand)
				: PutHome (HELPCOMMAND);
