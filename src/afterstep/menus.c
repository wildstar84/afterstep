...
{
	if (selection != menu->selected_item) {
/* JWT:STOP INVOKING SUBMENUS JUST B/C MOUSE MOVES SLIGHTLY BETWEEN PRESS AND
RELEASE (SLIGHT DRAG) AND RELEASE OCCURS ON AN AJACENT MENU-ITEM (VERY ANNOYING)!
TRYING TO NAVIGATE, INSTEAD, JUST MOVE THE SELECTION!
(ANNOYANCE OCCURS (OFTEN) WHEN CLICKING SCREEN TO INVOKE MAIN MENU WITHOUT HOLDING
MOUSE STEADY WHILST RELEASING!):
...
/*  JWT:A DOUBLE-COMPENSATION? - (MENU STARTS W/MOUSE OVER 2ND ITEM):  y -= tbar_height; */
/*  JWT:ONLY DO FOR TOPLEVEL MENU (START SUBMENUS AT MOUSE-POSITION):  x -= menu->optimal_width / 3; */

	if (menu->supermenu) {	 /* (SUBMENU): we need to make sure we would not overlay our parent completely ! */
		...
		x -= menu->optimal_width / 3;  /* JWT:MOVED HERE (DO ONLY FOR TOPLEVEL MENUS)! */
		if (x <= MIN_MENU_X)
			x = MIN_MENU_X;
